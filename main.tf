...
provider "proxmox" {
  ...
  pm_api_token_id = "terraform_user@pam!new_token_id"
...
resource "proxmox_vm_qemu" "kubernetes_server" {
  ...
  name = "kubernetes-${count.index + 1}" #count.index starts at 0, so + 1 means this VM will be named test-vm-1 in proxmox
